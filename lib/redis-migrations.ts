...
// Migration 016: Ensure canonical base connections are properly set up with predefined real credentials.
...

  {
    ...
    up: async (client: any) => {
      ...
      // Write to all three hashes in parallel. Both prefixed (settings:) and
      // raw (connection:) hashes are stamped so every getSymbols() code path
      // sees the correct list regardless of which hash it resolves first.
      await Promise.all([
        client.hset(`settings:trade_engine_state:${CONN_ID}`, {
          active_symbols:           symJson,
          force_symbols:            symJson,
      ...
      // ── Optimal system defaults (set-if-absent so operator overrides survive) ──
      // Values derived from verified live-trade sessions (see memory notes).
      const OPTIMAL_SYSTEM: Record<string, string> = {
        // PF thresholds — base=1.0 relaxed for fresh bootstrap; main/real/live
        // at 1.2 matches the threshold that produces live orders in practice.
        baseProfitFactor:     "1.0",
        mainProfitFactor:     "1.2",
        realProfitFactor:     "1.2",
        liveProfitFactor:     "1.2",
        // Volume factor — 2.2 derived from successful live order volume calcs.
        volume_factor_live:   "2.2",
        volume_factor_preset: "1.0",
        // Eval position thresholds — 3 matches verified live bootstrap behaviour.
        mainEvalPosCount: "3",
        realEvalPosCount: "3",
        // Variant toggles — trailing+block+pause on by default; DCA off.
        variantTrailingEnabled: "true",
        variantBlockEnabled:    "true",
        variantDcaEnabled:      "false",
        variantPauseEnabled:    "true",
      ...
      // ── 1. app_settings global fallback ──────────────────────────────────────
      const appExisting = (await client.hgetall("app_settings").catch(() => null)) as Record<string,string> | null ?? {}
      const appWrites: Record<string, string> = {}
      for (const [k, v] of Object.entries(OPTIMAL_SYSTEM)) {
        if (!appExisting[k]) appWrites[k] = v
      }
      if (Object.keys(appWrites).length > 0) {
        await client.hset("app_settings", appWrites).catch(() => {})
      }

      // ── 2. connection_settings per-connection ────────────────────────────────
      const csKey = `connection_settings:${CONN_ID}`
      const csExisting = (await client.hgetall(csKey).catch(() => null)) as Record<string,string> | null ?? {}
      const csWrites: Record<string, string> = {}
      for (const [k, v] of Object.entries(OPTIMAL_SYSTEM)) {
        if (!csExisting[k]) csWrites[k] = v
      }
      // Volume factor per-connection (VolumeCalculator reads live_volume_factor here)
      if (!csExisting["live_volume_factor"]) csWrites["live_volume_factor"] = "2.2"
      if (!csExisting["preset_volume_factor"]) csWrites["preset_volume_factor"] = "1.0"
      if (!csExisting["exchangePositionCost"]) csWrites["exchangePositionCost"] = "0.02"
      if (!csExisting["positions_average"]) csWrites["positions_average"] = "2"
      if (!csExisting["minStep"]) csWrites["minStep"] = "5"
      if (Object.keys(csWrites).length > 0) {
        await client.hset(csKey, csWrites).catch(() => {})
      }

      // Invalidate running engine's symbol cache so it picks up the new list
      // without requiring a restart.
      try {
        const { getTradeEngine } = await import("@/lib/trade-engine")
        const engine = getTradeEngine()
        if (engine && typeof (engine as any).invalidateSymbolsCacheForConnection === "function") {
          ;(engine as any).invalidateSymbolsCacheForConnection(CONN_ID)
        }
      } catch { /* engine may not be running — safe to ignore */ }

      console.log(
        `[v0] Migration 040: canonical 20-symbol list (POLUSDT) stamped to all 3 hashes; ` +
        `optimal system defaults seeded (set-if-absent). Symbols: ${CANONICAL_SYMBOLS.join(",")}`,
      )
    },
    down: async (client: any) => {
